...
describe('Alexa Who Am I Controller', () => {
  beforeEach((done) => {
    createDB(() => {
      ...
      .then((createdApplication) => {
        ...
        return LabFactory.createLab(company);
      })
      .then((createdLab) => {
        lab = createdLab;
  ...
  const intent = "WhoAmIIntent"

  it('should return a message about what\'s new', (done) => {
    const expectedOutput = {
      type: "SSML",
      ssml: "<speak>This device is setup for the Rhodes Lab</speak>"
    }
    const payload =  {
      "type": "IntentRequest",
